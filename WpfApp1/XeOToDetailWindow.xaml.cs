...
namespace WpfApp1
{
    public partial class XeOToDetailWindow : Window
    {
        private readonly int? _id;
        private readonly bool _readOnly;

        public XeOToDetailWindow(int? id, bool readOnly = false)
        {
            InitializeComponent();
            _id = id;
            _readOnly = readOnly;
        ...
        private void OnLoaded(object s, RoutedEventArgs e)
        {
            using (var db = new QuanlychungcuEntities())
            {
                ...
                else lblHeader.Text = "Thông tin xe ô tô (Thêm mới)";
            }
        ...
        private void BtnSave_Click(object s, RoutedEventArgs e)
        {
            ...
            using (var db = new QuanlychungcuEntities())
            {
                ...
                if (_id.HasValue)
                {
                    var entity = db.XeOToes.First(x => x.XeOToID == _id.Value);
                    entity.BKS = bks;
                    entity.CuDanID = cuDanId;
                }
                else db.XeOToes.Add(new XeOTo { BKS = bks, CuDanID = cuDanId });
